...
Human-facing portals remain useful in these deployments, but a portal is not a protocol contract. It does not by itself standardize automated submission, request correlation, browser interaction and return, deferred completion of a pending approval, secure credential delivery, audience restriction, the binding between reviewed metadata and the issued statement, renewal, or error handling. Interoperability requires independently implemented clients and issuers to agree on exactly those protocol surfaces, and deployment guidance cannot supply that agreement. This specification standardizes the protocol beneath the portal while leaving the approval workflow, acceptance policy, and establishment of issuer trust to the deployment.
...
The consumption side of this design is already standardized and deployed: {{RFC7591}} registration endpoints have accepted the `software_statement` request member since 2015. This specification supplies the missing issuance protocol and hardens the artifact so that independent issuers and consumers interoperate ({{software-statement-format}}). The resulting portability is bounded by trust: a statement is honored only where the trusting authorization server is configured to trust its issuer, so its practical audience is an ecosystem or administrative domain rather than the open web. This specification introduces no new client credential or federation architecture; it defines the missing lifecycle operations around an artifact already standardized for dynamic client registration, on protocol surfaces OAuth deployments already operate, leaving trust policy deployment-specific.

The protocol introduces `response_type=software_statement_code` at the authorization endpoint and one extension grant, `urn:ietf:params:oauth:grant-type:software-statement`, through which the client redeems the resulting software statement code at the token endpoint. Clients identify themselves with the HTTPS URL and metadata document defined by {{CIMD}}. The authorization server returns a short-lived software statement code; a request whose decision is still pending is deferred at redemption under {{DTR}} and resolved by polling, so approval can complete out of band over hours or days. A client that already holds a token carrying issuance authority, such as a qualifying initial access token issued at onboarding or an unexpired statement, instead uses OAuth token exchange directly at the token endpoint, with no user agent involved ({{token-exchange-profile}}).
...
OpenID Federation {{OPENID-FED}} also conveys third-party-attested client metadata across authorization servers, through entity statements resolved along trust chains anchored in a federation operator. That architecture suits ecosystems prepared to operate trust anchors and intermediates and to adopt its resolution model. This specification targets deployments below that threshold: it issues the {{RFC7591}} artifact that existing registration endpoints already accept, with issuer trust established by explicit pairwise configuration rather than chain resolution. Federation also standardizes resolution rather than acquisition: how an entity is enrolled with a trust anchor or intermediate, who approves that enrollment, and how a pending decision completes are out of its scope, precisely as issuance was out of scope for {{RFC7591}}. Adopting Federation therefore relocates the ceremony this specification defines rather than replacing it. The two address different scales of trust establishment and do not depend on each other.
...
This specification does not define the approval workflow, the identity of an approver, or a protocol between the authorization server and an external approval system. It also does not require a trusting authorization server to accept any particular issuer. Acceptance policy and trust establishment remain deployment-specific. Issuer discovery is likewise out of scope: how client software learns where to obtain a statement is deployment knowledge today. A natural future home for that association is the client's own metadata document, where a member naming the issuers that attest software under the publisher's namespace would double as the namespace-delegation evidence that {{TRUST-FRAMEWORK}} evaluates.
...
Classification uses the singular `token_endpoint_auth_method` member. This specification does not interpret a list of authentication methods a client merely declares it supports; a client that requires a deterministic public or confidential classification for issuance declares the singular `token_endpoint_auth_method`, and an authorization server classifies on that member alone.
...
This appendix is non-normative. It maps the protocol onto the delivery lifecycle of client software, using the deployment of an AI agent as the running example; the same mapping applies to any client software distributed beyond its publisher. The lifecycle events this specification standardizes, review, attestation, renewal, and expiry, are administrative: they sit between the supply-chain layer that attests how software was built and the runtime layers that identify instances and grant access.

Release:
: Each release updates the client's canonical metadata document, including the `software_version` and `software_id` members of {{RFC7591}} where used. Because every statement carries `cimd_digest` ({{software-statement-format}}), a statement functions as a signed, expiring release-approval artifact: exactly this metadata, for exactly this version, reviewed and attested for these audiences. A release pipeline obtains it without human interaction through the token exchange profile, presenting the credential issued at publisher onboarding ({{token-exchange-profile}}). The client identifier does not change with a release: updates change the document behind the stable client identifier URL, creating a new trust state for the same identity, and each statement binds the same `sub` to the digest of the content actually reviewed.

Distribution:
: A publisher program or marketplace reviews the software once and issues a statement whose audience covers the participating authorization servers ({{example-publisher}}). Ecosystems that identify clients by metadata document gain a machine-verifiable approval artifact in place of directories and manually maintained allowlists.
...
Upgrade:
: Renewal makes the upgrade protocol-visible. A new version changes the metadata document, so the issuer's fresh snapshot digest differs from the digest it recorded at the prior issuance, and issuance policy must decide explicitly whether the prior endorsement carries forward or the review re-runs ({{token-exchange-profile}}). The audit guidance in {{security-considerations}} binds that decision to the digests involved, so "was the new version re-approved or inherited" has a recorded answer.
...
Retirement:
: Sunset is the absence of renewal: when the publisher or deploying organization stops renewing, statements age out and no further registrations can be derived ({{statement-validation}}). Faster reaction composes through a `status` claim and issuer or namespace de-trust. Registrations already derived from a statement remain each trusting authorization server's client lifecycle to manage.

This mapping has deliberate limits: the statement does not attest binaries or build provenance, does not identify running instances ({{CLIENT-INSTANCE}}, {{ABCA}}), and grants no access. It standardizes the vouching layer of delivery, which no other layer of the stack defines.
...
Neither the software statement nor any long-lived credential should transit the front channel, and a pending decision must not imply a completion point that has not been reached. Redeeming a short-lived software statement code at the token endpoint satisfies both: an already-completed decision returns the statement immediately, and a pending one defers through {{DTR}} exactly as a token exchange does. Every deferral in this specification therefore originates from a token request, PKCE is consumed by ordinary redemption rather than a special first poll, and no extension to {{DTR}}'s authorization endpoint behavior is required.

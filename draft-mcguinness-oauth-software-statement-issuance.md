...
The `sub` claim identifies the client software by its client identifier URL; it does not name the `client_id` that a trusting authorization server will assign. Registration with the software statement proceeds under {{RFC7591}} and yields an authorization-server-specific `client_id` that need not be related to `sub`. A trusting authorization server MAY use `sub` to correlate registrations of the same client software and to apply per-client policy, such as bounding the number of active registrations ({{multi-instance}}). Because `sub` is stable across releases of the software, that correlation spans document revisions; the `cimd_digest` of each statement distinguishes the content that was reviewed. When the metadata snapshot includes `software_id`, that value identifies the software as defined by {{RFC7591}}; this specification does not derive either identifier from the other.
